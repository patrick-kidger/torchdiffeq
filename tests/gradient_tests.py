...
class TestGradient(unittest.TestCase):
    ...
    def test_midpoint(self):
        for device in devices:
            f, y0, t_points, _ = construct_problem(device)
...
class TestCompareAdjointGradient(unittest.TestCase):
    ...
    def problem(self):
        for device in devices:
            ...
            y0 = torch.tensor([[2., 0.]]).to(device).requires_grad_(True)
            t_points = torch.linspace(0., 25., 10).to(device).requires_grad_(True)

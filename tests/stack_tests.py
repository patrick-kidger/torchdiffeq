            ...
            for ode in ('constant', 'linear', 'sine'):
                ...
                for _ in range(20):
                    ...
                    for odeint_ in (torchdiffeq.odeint, torchdiffeq.odeint_adjoint):
                        ...
                        self.assertLess(rel_error(y0_grad, y0_grad_), error_tol)
                        # For some reason this can get quite large errors.
                        # Not sure why.
                        self.assertLess(rel_error(t_points_grad, t_points_grad_), 10 * error_tol)
                        for param, param_grad in zip(f.parameters(), param_grads):
                            param_grad_ = param.grad
                            self.assertLess(rel_error(param_grad, param_grad_), error_tol)
